...
class JsTreeBehavior extends CBehavior
{
    ...
    public function actionMoveCopy()
    {
        ...
        if ($new_parent_root_id != 'root') {
            ...
            //else if it is a copy
            	...
            	$copied_node->appendTo($new_parent);
            	
            	
            	$this->CopyWholeSubTree($copied_node, $moved_node);
    ...
    private function CopyWholeSubTree(&$copied_node, &$moved_node){
    	
    	foreach($moved_node->children()->findAll() as $child){
    		
    		$newNode = new $this->modelClassName;
    		$newNode->attributes = $child->attributes;
            $newNode->primaryKey = null;
    		
    		$newNode->appendTo($copied_node);
    		$this->CopyWholeSubTree($newNode, $child);
